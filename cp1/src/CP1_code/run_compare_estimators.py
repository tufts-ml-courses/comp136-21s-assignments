...
if __name__ == '__main__':
    ...
    # fit ML Estimator on train, then score it on the test set
    for i in range(len(n_train_list)):
        mle = MLEstimator(vocab)
        mle.fit(train_word_list[:n_train_list[i]])
        mle_scores[i] += mle.score(test_word_list)

    # fit MAP Estimator on train, then score it on the test set
    for i in range(len(n_train_list)):
        map = MAPEstimator(vocab)
        map.fit(train_word_list[:n_train_list[i]])
        map_scores[i] += map.score(test_word_list)

    # fit PosteriorPredictive Estimator on train, then score it on the test set
    for i in range(len(n_train_list)):
        ppe = PosteriorPredictiveEstimator(vocab)
        ppe.fit(train_word_list[:n_train_list[i]])
