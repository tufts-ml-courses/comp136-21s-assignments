    ...
    * 2) does not give zero probability to any word
    ...
    def fit(self, word_list):
        ...
        * The integer total_count is set to the total length of the word list
        ...
        self.total_count = 0
        self.unseen_count = 0
        # update total_count and unseen_count
        self.total_count = len(word_list)
        self.unseen_count = self.vocab.size - len(set(word_list))
    ...
    def predict_proba(self, word):
        ...
        if self.count_V[id] > 0:
            return (1 - self.unseen_proba) * self.count_V[id] / self.total_count
        else:
